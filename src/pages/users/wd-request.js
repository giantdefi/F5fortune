...
export default function Users() {
   ...
   const handleWDRequest = (item) => {
    
    setSpinner(true)
   const data = {
        wdWalletAmount,walletAddr, userid 
        }
       ...
       if(!wdWalletAmount){
        ...
        return dispatch(setError({ path: "wdWalletAmount", message: 'WD amount is required' }))
       }
       if(!walletAddr){
        setSpinner(false)
        dispatch(setPlaySound('error'))
        return dispatch(setError({ path: "walletAddr", message: 'Wallet address is required' }))
       }
...
<div className="flex justify-center mt-10">
    {spinner ?
        ...
        <button onClick={handleWDRequest}
            className="_btn_submit_green w-[90%] mb-5 text-xl mx-auto py-2 mt-5 border-4 border-gray-200  flex justify-center items-center">
